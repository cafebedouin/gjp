...
acled_riots <- function(country="Gabon") {
  ...
  getACLED <- function(country = "NULL", year = "NULL") {
    ...
    if(is.null(country2) & is.null(year)) {
      myurl <- "https://api.acleddata.com/acled/read?&limit=500"
    ...
    # Download the data! The API returns a list in JSON format:
    acledEvents <- httr::GET(paste0(myurl), accept_json()) 
    
    acledEvents <- fromJSON(acledEvents)
  ...
  data_riots <- getACLED(country = "Afghanistan")
