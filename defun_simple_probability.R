...
# with two columns (date, value). The first as.Date,
...
defun_simple_probability <- function(df,
  ...
  # Get the length of df$value and shorten it by remaining_days
  prob_rows = length(df$value) - remaining_days
  ...
  for (i in 1:prob_rows) {
    prob_calc[i] <- df$value[i] - df$value[i+remaining_days]
  }
